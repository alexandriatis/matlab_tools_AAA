function pretty_plotter_GM(f,ma,nu,plot_title,lat,N2) 
% Makes a nice spectral plot along with a GM spectrum
% f = frequency axis in cycles per day
% ma = spectral amplitude, same dimensions as f
% nu = number of degrees of freedom for chi squared confidence intervals
% plot_title = a string for the plot title
% lat = latitude in degrees (-90 to 90)
% N2 = Buoyancy frequency squared (rad/sec)^2
...
colors; %This is a function of pretty colors I made
   ...
   % Overlay the GM spectrum
        frad = sw_f(lat); % rad/sec
        Nrad = sqrt(N2); % rad/sec
        om = linspace(frad,Nrad,10000);
        S_GM = GmOm('Vel',om,frad,Nrad,Gm76Params); % Not quite sure this is the right normalization because I'm using just U instead of 'Vel' but 
        % I'm not sure what 'Vel' = 'U'+'V' means
        S_GM=S_GM/86400*2*pi/2;  %Convert to cycles per day and divide by half since only one component of velocity
        om=om*86400/(2*pi);%cpd
        loglog(om,S_GM,'k');

    % Overlay tidal frequency indicators    
